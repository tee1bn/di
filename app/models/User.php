...
class User extends Eloquent 
{
    ...
    public function urgent_match()
    {
        ...
        $urgent_match =  Match::whereIn('ph_id', $phs_ids)
                                ->whereDate('expires', '>',date("Y-m-d"))
                                ->where('status', null)->orderBy('expires', 'asc')->first();
    ...
    public function scopePioneers($query)
    {
    	...
    	return $query->where('account_plan', 'pioneer');
	...
	public function PhRequests()
	{

		return $this->hasMany('PH','user_id');
	...
	public function GhRequests()
	{

		return $this->hasMany('GH','user_id');
...
public static function generate_phone_code_for($user_id)
{
    ...
    public function getAdminViewUrlAttribute()
    {
    	...
    	return  Config::domain()."/admin/user_profile/".$this->id;
    }
    ...
    //keeper users
	public function demo_status()
	{
		if ($this->account_plan == 'pioneer') {
			return "<span class='badge badge-primary fa fa-star'> Pioneer</span>";
		}

		return false;
			// return "<span class='label label-success'>DEMO</span>"
	}

	public function isnot_demo_user()
	{
		return ($this->account_plan != 'pioneer');
	}
    public function rejoin_if_ripe()
    {
    	if ($this->ripe_for_rejoin()) {
    		$this->rejoin();
    	}
    }
